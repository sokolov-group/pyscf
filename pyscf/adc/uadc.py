...
def get_imds_ip_cvs(adc, eris=None):
    ...
    #print("eris_ceeo: ", eris_ceeo.flags.c_contiguous)  
    #print("eris_CEEO: ", eris_CEEO.flags.c_contiguous)
    #print("eris_ceEO: ", eris_ceEO.flags.c_contiguous)
    #print("eris_oeEC: ", eris_oeEC.flags.c_contiguous)
    #exit()
    ...
    M_ij_a += 0.5 * 0.5 *  lib.einsum('ilde,jdel->ij',t2_1_a_coee, eris_ceeo, optimize=True)
    M_ij_a -= 0.5 * 0.5 *  lib.einsum('ilde,jedl->ij',t2_1_a_coee, eris_ceeo, optimize=True)
    M_ij_a += 0.5 * 0.5 *  lib.einsum('jlde,idel->ij',t2_1_a_coee, eris_ceeo, optimize=True)
    M_ij_a -= 0.5 * 0.5 *  lib.einsum('jlde,iedl->ij',t2_1_a_coee, eris_ceeo, optimize=True)

    t2_1_b = t2[0][2][:]
    t2_1_b_coee = t2_1_b[:ncvs,:,:,:].copy()
    M_ij_b += 0.5 * 0.5 *  lib.einsum('ilde,jdel->ij',t2_1_b_coee, eris_CEEO, optimize=True)
    M_ij_b -= 0.5 * 0.5 *  lib.einsum('ilde,jedl->ij',t2_1_b_coee, eris_CEEO, optimize=True)
    M_ij_b += 0.5 * 0.5 *  lib.einsum('jlde,idel->ij',t2_1_b_coee, eris_CEEO, optimize=True)
    M_ij_b -= 0.5 * 0.5 *  lib.einsum('jlde,iedl->ij',t2_1_b_coee, eris_CEEO, optimize=True)

    t2_1_ab = t2[0][1][:]
    t2_1_ab_coee = t2_1_ab[:ncvs,:,:,:].copy()
    t2_1_ab_ocee = t2_1_ab[:,:ncvs,:,:].copy()
    M_ij_a += 0.5 * lib.einsum('ilde,jdel->ij',t2_1_ab_coee, eris_ceEO, optimize=True)
    M_ij_b += 0.5 * lib.einsum('lied,ledj->ij',t2_1_ab_ocee, eris_oeEC, optimize=True)
    M_ij_a += 0.5 * lib.einsum('jlde,idel->ij',t2_1_ab_coee, eris_ceEO, optimize=True)
    M_ij_b += 0.5 * lib.einsum('ljed,ledi->ij',t2_1_ab_ocee, eris_oeEC, optimize=True)
    ...
    if (method == "adc(3)"):
        ...
        #print("eris_ceec: ", eris_ceec.flags.c_contiguous)        
        #print("eris_CEEC: ", eris_CEEC.flags.c_contiguous)
        #print("eris_ocee: ", eris_ocee.flags.c_contiguous)
        #print("eris_ccee: ", eris_ccee.flags.c_contiguous)
        #print("eris_OCEE: ", eris_OCEE.flags.c_contiguous)
        #print("eris_CCEE: ", eris_CCEE.flags.c_contiguous)
        #print("eris_ocEE: ", eris_ocEE.flags.c_contiguous)
        #print("eris_ccEE: ", eris_ccEE.flags.c_contiguous)
        #print("eris_OCee: ", eris_OCee.flags.c_contiguous)
        #print("eris_CCee: ", eris_CCee.flags.c_contiguous)
        #print("eris_oecc: ", eris_oecc.flags.c_contiguous)
        #print("eris_ceoc: ", eris_ceoc.flags.c_contiguous)
        #print("eris_OECC: ", eris_OECC.flags.c_contiguous)
        #print("eris_CEOC: ", eris_CEOC.flags.c_contiguous)
        #print("eris_oeCC: ", eris_oeCC.flags.c_contiguous)
        #print("eris_OEcc: ", eris_OEcc.flags.c_contiguous)
        #print("eris_cooo: ", eris_cooo.flags.c_contiguous)
        #print("eris_ccoo: ", eris_ccoo.flags.c_contiguous)
        #print("eris_cooc: ", eris_cooc.flags.c_contiguous)
        #print("eris_COOO: ", eris_COOO.flags.c_contiguous)
        #print("eris_CCOO: ", eris_CCOO.flags.c_contiguous)
        #print("eris_COOC: ", eris_COOC.flags.c_contiguous)
        #print("eris_ccOO: ", eris_ccOO.flags.c_contiguous)
        #print("eris_ooCC: ", eris_ooCC.flags.c_contiguous)
        #print("eris_coOO: ", eris_coOO.flags.c_contiguous)
        #print("eris_ooOC: ", eris_ooOC.flags.c_contiguous)
        #exit()
        ...
        M_ij_a += 0.5 * 0.5* lib.einsum('ilde,jdel->ij',t2_2_a_coee, eris_ceeo, optimize=True)
        M_ij_a -= 0.5 * 0.5* lib.einsum('ilde,jedl->ij',t2_2_a_coee, eris_ceeo, optimize=True)

        M_ij_a += 0.5 * 0.5* lib.einsum('jlde,idel->ij',t2_2_a_coee, eris_ceeo, optimize=True)
        M_ij_a -= 0.5 * 0.5* lib.einsum('jlde,iedl->ij',t2_2_a_coee, eris_ceeo, optimize=True)
        ...
        M_ij_b += 0.5 * 0.5* lib.einsum('ilde,jdel->ij',t2_2_b_coee, eris_CEEO, optimize=True)
        M_ij_b -= 0.5 * 0.5* lib.einsum('ilde,jedl->ij',t2_2_b_coee, eris_CEEO, optimize=True)
        M_ij_b += 0.5 * 0.5* lib.einsum('jlde,idel->ij',t2_2_b_coee, eris_CEEO, optimize=True)
        M_ij_b -= 0.5 * 0.5* lib.einsum('jlde,iedl->ij',t2_2_b_coee, eris_CEEO, optimize=True)
        ...
        M_ij_a += 0.5 * lib.einsum('ilde,jdel->ij',t2_2_ab_coee, eris_ceEO, optimize=True)
        M_ij_b += 0.5 * lib.einsum('lied,ledj->ij',t2_2_ab_ocee, eris_oeEC, optimize=True)
        M_ij_a += 0.5 * lib.einsum('jlde,idel->ij',t2_2_ab_coee, eris_ceEO, optimize=True)
        M_ij_b += 0.5 * lib.einsum('ljed,ledi->ij',t2_2_ab_ocee, eris_oeEC, optimize=True)

        M_ij_t = lib.einsum('lmde,jldf->mejf', t2_1_a, t2_1_a_coee, optimize=True)
        M_ij_a -= 0.5 * lib.einsum('mejf,ifem->ij',M_ij_t, eris_ceeo, optimize=True)
        M_ij_a -= 0.5 * lib.einsum('mejf,ifem->ji',M_ij_t, eris_ceeo, optimize=True)
        M_ij_a += 0.5 * lib.einsum('mejf,mife->ij',M_ij_t, eris_ocee, optimize=True)
        M_ij_a += 0.5 * lib.einsum('mejf,mife->ji',M_ij_t, eris_ocee, optimize=True)
        del M_ij_t
        ...
        M_ij_a -= 0.5*lib.einsum('lmdf,lmde,jfei->ij',t2_1_a, t2_1_a, eris_ceec, optimize = True)
        ...
        M_ij_a += 0.5*lib.einsum('lnde,lmde,jmni->ij',t2_1_a, t2_1_a, eris_cooc, optimize = True)
        M_ij_b -= 0.5 * lib.einsum('lnde,lmde,nmji->ij',t2_1_a, t2_1_a, eris_ooCC, optimize = True)

        t2_1_ab = t2[0][1][:]
        t2_1_ab_coee = t2_1_ab[:ncvs,:,:,:].copy()
        t2_1_ab_ocee = t2_1_ab[:,:ncvs,:,:].copy()
        M_ij_a -= 0.5 * lib.einsum('lmde,jldf,ifem->ij',t2_1_ab, t2_1_a_coee, eris_ceEO,optimize = True)
        M_ij_b += 0.5 * lib.einsum('lmde,ljdf,mefi->ij',t2_1_a, t2_1_ab_ocee, eris_oeEC ,optimize = True)
        M_ij_a -= 0.5 * lib.einsum('lmde,ildf,jfem->ij',t2_1_ab, t2_1_a_coee, eris_ceEO,optimize = True)
        M_ij_b += 0.5 * lib.einsum('lmde,lidf,mefj->ij',t2_1_a, t2_1_ab_ocee, eris_oeEC ,optimize = True)
        del t2_1_a
        del t2_1_a_coee

        M_ij_a += 0.5 * lib.einsum('lmde,jlfd,ifem->ij',t2_1_b, t2_1_ab_coee, eris_ceEO ,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('mled,jldf,mefi->ij',t2_1_ab, t2_1_b_coee, eris_oeEC,optimize = True)
        M_ij_a += 0.5 * lib.einsum('lmde,ilfd,jfem->ij',t2_1_b, t2_1_ab_coee, eris_ceEO ,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('mled,ildf,mefj->ij',t2_1_ab, t2_1_b_coee, eris_oeEC,optimize = True)

        M_ij_t = lib.einsum('lmde,jldf->mejf', t2_1_b, t2_1_b_coee, optimize=True)
        M_ij_b -= 0.5 * lib.einsum('mejf,ifem->ij',M_ij_t, eris_CEEO, optimize=True)
        M_ij_b -= 0.5 * lib.einsum('mejf,ifem->ji',M_ij_t, eris_CEEO, optimize=True)
        M_ij_b += 0.5 * lib.einsum('mejf,mife->ij',M_ij_t, eris_OCEE, optimize=True)
        M_ij_b += 0.5 * lib.einsum('mejf,mife->ji',M_ij_t, eris_OCEE, optimize=True)
        del M_ij_t
        ...
        M_ij_b -= 0.5*lib.einsum('lmdf,lmde,jfei->ij',t2_1_b, t2_1_b, eris_CEEC , optimize = True)
        ...
        M_ij_b += 0.5*lib.einsum('lnde,lmde,jmni->ij',t2_1_b, t2_1_b, eris_COOC, optimize = True)
        del t2_1_b
        del t2_1_b_coee

        M_ij_a += 0.5 * lib.einsum('mled,jlfd,ifem->ij',t2_1_ab, t2_1_ab_coee, eris_ceeo ,optimize = True)
        M_ij_a -= 0.5 * lib.einsum('mled,jlfd,mife->ij',t2_1_ab, t2_1_ab_coee, eris_ocee ,optimize = True)
        M_ij_a -= 0.5 * lib.einsum('mlde,jldf,mife->ij',t2_1_ab, t2_1_ab_coee, eris_ocEE ,optimize = True)

        M_ij_b += 0.5 * lib.einsum('lmde,ljdf,ifem->ij',t2_1_ab, t2_1_ab_ocee, eris_CEEO,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('lmde,ljdf,mife->ij',t2_1_ab, t2_1_ab_ocee, eris_OCEE,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('lmed,ljfd,mife->ij',t2_1_ab, t2_1_ab_ocee, eris_OCee ,optimize = True)

        M_ij_a += 0.5 * lib.einsum('mled,ilfd,jfem->ij',t2_1_ab, t2_1_ab_coee, eris_ceeo ,optimize = True)
        M_ij_a -= 0.5 * lib.einsum('mled,ilfd,mjfe->ij',t2_1_ab, t2_1_ab_coee, eris_ocee ,optimize = True)
        M_ij_a -= 0.5 * lib.einsum('mlde,ildf,mjfe->ij',t2_1_ab, t2_1_ab_coee, eris_ocEE ,optimize = True)

        M_ij_b += 0.5 * lib.einsum('lmde,lidf,jfem->ij',t2_1_ab, t2_1_ab_ocee, eris_CEEO ,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('lmde,lidf,mjfe->ij',t2_1_ab, t2_1_ab_ocee, eris_OCEE ,optimize = True)
        M_ij_b -= 0.5 * lib.einsum('lmed,lifd,mjfe->ij',t2_1_ab, t2_1_ab_ocee, eris_OCee ,optimize = True)

        M_ij_a += 0.5 * lib.einsum('lmde,jnde,ilmn->ij',t2_1_ab ,t2_1_ab_coee,eris_coOO, optimize = True)
        M_ij_b += 0.5 * lib.einsum('mled,njed,mnli->ij',t2_1_ab ,t2_1_ab_ocee,eris_ooOC, optimize = True)

        M_ij_a +=0.5 * lib.einsum('inde,lmde,jlnm->ij',t2_1_ab_coee, t2_1_ab,eris_coOO, optimize = True)

        M_ij_b +=0.5 * lib.einsum('nied,mled,nmlj->ij',t2_1_ab_ocee, t2_1_ab,eris_ooOC, optimize = True)

        M_ij_a +=lib.einsum('mlfd,mled,jief->ij',t2_1_ab, t2_1_ab, eris_ccee , optimize = True)
        M_ij_a -=lib.einsum('mlfd,mled,jfei->ij',t2_1_ab, t2_1_ab, eris_ceec , optimize = True)
        ...
        M_ij_b -=lib.einsum('lmdf,lmde,jfei->ij',t2_1_ab, t2_1_ab, eris_CEEC , optimize = True)
        ...
        M_ij_a += lib.einsum('nled,mled,jmni->ij',t2_1_ab, t2_1_ab, eris_cooc, optimize = True)
        ...
        M_ij_b += lib.einsum('lnde,lmde,jmni->ij',t2_1_ab, t2_1_ab, eris_COOC, optimize = True)
...
#@profile
def ip_cvs_adc_matvec_off_np(adc, M_ij=None, eris=None):
    # numpy einsums
    if adc.method not in ("adc(2)", "adc(2)-x", "adc(3)"):
        raise NotImplementedError(adc.method)
...
############# ADC(2) i - kja block #########################
        ...
        s[s_a:f_a] += lib.einsum('JakI,aJk->I', eris_cevc, r_aaa_ecv, optimize = True)
        ...
        s[s_a:f_a] += lib.einsum('JakI,aJk->I', eris_CEvc, r_bba_ecv, optimize = True)
        ...
        s[s_b:f_b] += lib.einsum('JakI,aJk->I', eris_CEVC, r_bbb_ecv, optimize = True)
        ...
        s[s_b:f_b] += lib.einsum('JakI,aJk->I', eris_ceVC, r_aab_ecv, optimize = True)
...
############## ADC(2) ajk - i block ############################
        ...
        s[s_aaa_ecv:f_aaa_ecv] += lib.einsum('JakI,I->aJk', eris_cevc, r_a, optimize = True).reshape(-1)
        ...
        s[s_bba_ecv:f_bba_ecv] += lib.einsum('JakI,I->aJk', eris_CEvc, r_a, optimize = True).reshape(-1)
        ...
        s[s_aab_ecv:f_aab_ecv] += lib.einsum('JakI,I->aJk', eris_ceVC, r_b, optimize = True).reshape(-1)
        ...
        s[s_bbb_ecv:f_bbb_ecv] += lib.einsum('JakI,I->aJk', eris_CEVC, r_b, optimize = True).reshape(-1)
...
############### ADC(3) ajk - bil block ############################
        ...
        if (method == "adc(2)-x" or method == "adc(3)"):
               ...
               temp_ecc =  0.5*np.einsum('JLKI,aIL->aJK',eris_cccc,r_aaa_ecc_u ,optimize = True)
               temp_ecc -= 0.5*np.einsum('JIKL,aIL->aJK',eris_cccc,r_aaa_ecc_u ,optimize = True)
               temp_ecc +=     np.einsum('KIJl,aIl->aJK',eris_cccv,r_aaa_ecv ,optimize = True)
               temp_ecc -=     np.einsum('JIKl,aIl->aJK',eris_cccv,r_aaa_ecv ,optimize = True)
               temp_ecv =  0.5*np.einsum('JLIk,aIL->aJk',eris_cccv,r_aaa_ecc_u ,optimize = True)
               temp_ecv -= 0.5*np.einsum('JILk,aIL->aJk',eris_cccv,r_aaa_ecc_u ,optimize = True)
               temp_ecv +=     np.einsum('JlkI,aIl->aJk',eris_cvvc,r_aaa_ecv ,optimize = True)
               temp_ecv -=     np.einsum('JIkl,aIl->aJk',eris_ccvv,r_aaa_ecv ,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)

               temp_ecc =  0.5*np.einsum('JLKI,aIL->aJK',eris_CCCC,r_bbb_ecc_u ,optimize = True)
               temp_ecc -= 0.5*np.einsum('JIKL,aIL->aJK',eris_CCCC,r_bbb_ecc_u ,optimize = True)
               temp_ecc +=     np.einsum('KIJl,aIl->aJK',eris_CCCV,r_bbb_ecv ,optimize = True)
               temp_ecc -=     np.einsum('JIKl,aIl->aJK',eris_CCCV,r_bbb_ecv ,optimize = True)
               temp_ecv =  0.5*np.einsum('JLIk,aIL->aJk',eris_CCCV,r_bbb_ecc_u ,optimize = True)
               temp_ecv -= 0.5*np.einsum('JILk,aIL->aJk',eris_CCCV,r_bbb_ecc_u ,optimize = True)
               temp_ecv +=     np.einsum('JlkI,aIl->aJk',eris_CVVC,r_bbb_ecv ,optimize = True)
               temp_ecv -=     np.einsum('JIkl,aIl->aJk',eris_CCVV,r_bbb_ecv ,optimize = True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)

               s[s_bba_ecc:f_bba_ecc] -= np.einsum('KLJI,aIL->aJK',eris_ccCC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -= np.einsum('KIJl,alI->aJK',eris_ccCV,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -= np.einsum('lKJI,aIl->aJK',eris_vcCC,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -= np.einsum('kLJI,aIL->aJk',eris_vcCC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -= np.einsum('kIJl,alI->aJk',eris_vcCV,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -= np.einsum('klJI,aIl->aJk',eris_vvCC,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -= np.einsum('KLIj,aIL->ajK',eris_ccCV,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -= np.einsum('KIjl,alI->ajK',eris_ccVV,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -= np.einsum('lKIj,aIl->ajK',eris_vcCV,r_bba_ecv,optimize = True).reshape(-1)
                
               s[s_aab_ecc:f_aab_ecc] -= np.einsum('JIKL,aIL->aJK',eris_ccCC,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= np.einsum('lJKI,alI->aJK',eris_vcCC,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= np.einsum('JIKl,aIl->aJK',eris_ccCV,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= np.einsum('JILk,aIL->aJk',eris_ccCV,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= np.einsum('lJIk,alI->aJk',eris_vcCV,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= np.einsum('JIkl,aIl->aJk',eris_ccVV,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= np.einsum('jIKL,aIL->ajK',eris_vcCC,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= np.einsum('jlKI,alI->ajK',eris_vvCC,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= np.einsum('jIKl,aIl->ajK',eris_vcCV,r_aab_ecv,optimize = True).reshape(-1)
               """ 
               # Einsum paths testing 
               path_ccee = np.einsum_path('KLba,bJL->aJK',eris_ccee,r_aaa_ecc_u,optimize = True)
               path_ceec = np.einsum_path('KabL,bJL->aJK',eris_ceec,r_aaa_ecc_u,optimize = True)
               print("path info for: ", adc.method_type)
               print("path info for 'KLba,bJL->aJK',eris_ccee,r_aaa_ecc_u")
               print(path_ccee[0])
               print(path_ccee[1])
               print("path info for 'KabL,bJL->aJK',eris_ceec,r_aaa_ecc_u")
               print(path_ceec[0])
               print(path_ceec[1])
               """
               temp_ecc =  0.5*np.einsum('KLba,bJL->aJK',eris_ccee,r_aaa_ecc_u,optimize = True)
               temp_ecc -= 0.5*np.einsum('KabL,bJL->aJK',eris_ceec,r_aaa_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('LbaK,bLJ->aJK',eris_CEec,r_bba_ecc,optimize = True)
               temp_ecc += 0.5*np.einsum('lKba,bJl->aJK',eris_vcee,r_aaa_ecv,optimize = True)
               temp_ecc -= 0.5*np.einsum('lbaK,bJl->aJK',eris_veec,r_aaa_ecv,optimize = True)
               temp_ecc += 0.5*np.einsum('lbaK,blJ->aJK',eris_VEec,r_bba_evc,optimize = True)
               temp_ecv =  0.5*np.einsum('kLba,bJL->aJk',eris_vcee,r_aaa_ecc_u,optimize = True)
               temp_ecv -= 0.5*np.einsum('kabL,bJL->aJk',eris_veec,r_aaa_ecc_u,optimize = True)
               temp_ecv += 0.5*np.einsum('Lbak,bLJ->aJk',eris_CEev,r_bba_ecc,optimize = True)
               temp_ecv += 0.5*np.einsum('klba,bJl->aJk',eris_vvee,r_aaa_ecv,optimize = True)
               temp_ecv -= 0.5*np.einsum('kabl,bJl->aJk',eris_veev,r_aaa_ecv,optimize = True)
               temp_ecv += 0.5*np.einsum('lbak,blJ->aJk',eris_VEev,r_bba_evc,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
              
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('KLba,bJL->aJK',eris_ccEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('lKba,bJl->aJK',eris_vcEE,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += 0.5*np.einsum('kLba,bJL->aJk',eris_vcEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += 0.5*np.einsum('klba,bJl->aJk',eris_vvEE,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] += 0.5*np.einsum('KLba,bjL->ajK',eris_ccEE,r_bba_evc,optimize = True).reshape(-1)
               
               temp_1_ecc =  0.5*np.einsum('KLba,bJL->aJK',eris_CCEE,r_bbb_ecc_u,optimize = True)
               temp_1_ecc -= 0.5*np.einsum('KabL,bJL->aJK',eris_CEEC,r_bbb_ecc_u,optimize = True)
               temp_1_ecc += 0.5*np.einsum('KabL,bLJ->aJK',eris_CEec,r_aab_ecc,optimize = True)
               temp_1_ecc += 0.5*np.einsum('lKba,bJl->aJK',eris_VCEE,r_bbb_ecv,optimize = True)
               temp_1_ecc -= 0.5*np.einsum('lbaK,bJl->aJK',eris_VEEC,r_bbb_ecv,optimize = True)
               temp_1_ecc += 0.5*np.einsum('Kabl,blJ->aJK',eris_CEev,r_aab_evc,optimize = True)
               temp_1_ecv =  0.5*np.einsum('kLba,bJL->aJk',eris_VCEE,r_bbb_ecc_u,optimize = True)
               temp_1_ecv -= 0.5*np.einsum('kabL,bJL->aJk',eris_VEEC,r_bbb_ecc_u,optimize = True)
               temp_1_ecv += 0.5*np.einsum('kabL,bLJ->aJk',eris_VEec,r_aab_ecc,optimize = True)
               temp_1_ecv += 0.5*np.einsum('klba,bJl->aJk',eris_VVEE,r_bbb_ecv,optimize = True)
               temp_1_ecv -= 0.5*np.einsum('kabl,bJl->aJk',eris_VEEV,r_bbb_ecv,optimize = True)
               temp_1_ecv += 0.5*np.einsum('kabl,blJ->aJk',eris_VEev,r_aab_evc,optimize = True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_1_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_1_ecv.reshape(-1)
               

               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('KLba,bJL->aJK',eris_CCee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('lKba,bJl->aJK',eris_VCee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('kLba,bJL->aJk',eris_VCee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('klba,bJl->aJk',eris_VVee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('KLba,bjL->ajK',eris_CCee,r_aab_evc,optimize = True).reshape(-1)

               temp_ecc =  -0.5*np.einsum('JLba,bKL->aJK',eris_ccee,r_aaa_ecc_u,optimize = True)
               temp_ecc +=  0.5*np.einsum('JabL,bKL->aJK',eris_ceec,r_aaa_ecc_u,optimize = True)
               temp_ecc -=  0.5*np.einsum('LbaJ,bLK->aJK',eris_CEec,r_bba_ecc,optimize = True)
               temp_ecc += -0.5*np.einsum('lJba,bKl->aJK',eris_vcee,r_aaa_ecv,optimize = True)
               temp_ecc +=  0.5*np.einsum('lbaJ,bKl->aJK',eris_veec,r_aaa_ecv,optimize = True)
               temp_ecc -=  0.5*np.einsum('lbaJ,blK->aJK',eris_VEec,r_bba_evc,optimize = True)
               temp_ecv =   0.5*np.einsum('JLba,bLk->aJk',eris_ccee,r_aaa_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('JabL,bLk->aJk',eris_ceec,r_aaa_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('LbaJ,bLk->aJk',eris_CEec[:ncvs,:,:,:ncvs],r_bba_ecv,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
               

               s[s_bba_ecc:f_bba_ecc] +=  0.5*np.einsum('JabL,bKL->aJK',eris_CEec,r_aaa_ecc_u,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] +=  0.5*np.einsum('JLba,bLK->aJK',eris_CCEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -=  0.5*np.einsum('JabL,bLK->aJK',eris_CEEC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] +=  0.5*np.einsum('Jabl,bKl->aJK',eris_CEev,r_aaa_ecv,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] +=  0.5*np.einsum('lJba,blK->aJK',eris_VCEE,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -=  0.5*np.einsum('lbaJ,blK->aJK',eris_VEEC,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += -0.5*np.einsum('JabL,bLk->aJk',eris_CEec,r_aaa_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] +=  0.5*np.einsum('JLba,bLk->aJk',eris_CCEE,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -=  0.5*np.einsum('JabL,bLk->aJk',eris_CEEC,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] +=  0.5*np.einsum('jabL,bKL->ajK',eris_VEec,r_aaa_ecc_u,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] +=  0.5*np.einsum('jLba,bLK->ajK',eris_VCEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -=  0.5*np.einsum('jabL,bLK->ajK',eris_VEEC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] +=  0.5*np.einsum('jabl,bKl->ajK',eris_VEev,r_aaa_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] +=  0.5*np.einsum('jlba,blK->ajK',eris_VVEE,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -=  0.5*np.einsum('jabl,blK->ajK',eris_VEEV,r_bba_evc,optimize = True).reshape(-1)

               temp_ecc = -0.5*np.einsum('JLba,bKL->aJK',eris_CCEE,r_bbb_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('JabL,bKL->aJK',eris_CEEC,r_bbb_ecc_u,optimize = True)
               temp_ecc -= 0.5*np.einsum('JabL,bLK->aJK',eris_CEec,r_aab_ecc,optimize = True)
               temp_ecc +=-0.5*np.einsum('lJba,bKl->aJK',eris_VCEE,r_bbb_ecv,optimize = True)
               temp_ecc += 0.5*np.einsum('lbaJ,bKl->aJK',eris_VEEC,r_bbb_ecv,optimize = True)
               temp_ecc -= 0.5*np.einsum('Jabl,blK->aJK',eris_CEev,r_aab_evc,optimize = True)
               temp_ecv =  0.5*np.einsum('JLba,bLk->aJk',eris_CCEE,r_bbb_ecv,optimize = True)
               temp_ecv +=-0.5*np.einsum('JabL,bLk->aJk',eris_CEEC,r_bbb_ecv,optimize = True)
               temp_ecv -= 0.5*np.einsum('JabL,bLk->aJk',eris_CEec,r_aab_ecv,optimize = True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)
               
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('JLba,bLK->aJK',eris_ccee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= 0.5*np.einsum('JabL,bLK->aJK',eris_ceec,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('LbaJ,bKL->aJK',eris_CEec,r_bbb_ecc_u,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('lJba,blK->aJK',eris_vcee,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= 0.5*np.einsum('lbaJ,blK->aJK',eris_veec,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('lbaJ,bKl->aJK',eris_VEec,r_bbb_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('JLba,bLk->aJk',eris_ccee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= 0.5*np.einsum('JabL,bLk->aJk',eris_ceec,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] +=-0.5*np.einsum('LbaJ,bLk->aJk',eris_CEec,r_bbb_ecv,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('jLba,bLK->ajK',eris_vcee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= 0.5*np.einsum('jabL,bLK->ajK',eris_veec,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('Lbaj,bKL->ajK',eris_CEev,r_bbb_ecc_u,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('jlba,blK->ajK',eris_vvee,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= 0.5*np.einsum('jabl,blK->ajK',eris_veev,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('lbaj,bKl->ajK',eris_VEev,r_bbb_ecv,optimize = True).reshape(-1)
                
               temp_ecc = -0.5*np.einsum('KIba,bIJ->aJK',eris_ccee,r_aaa_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('KabI,bIJ->aJK',eris_ceec,r_aaa_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('IbaK,bIJ->aJK',eris_CEec,r_bba_ecc,optimize = True)
               temp_ecc += 0.5*np.einsum('iKba,bJi->aJK',eris_vcee,r_aaa_ecv,optimize = True)
               temp_ecc +=-0.5*np.einsum('ibaK,bJi->aJK',eris_veec,r_aaa_ecv,optimize = True)
               temp_ecc += 0.5*np.einsum('ibaK,biJ->aJK',eris_VEec,r_bba_evc,optimize = True)
               temp_ecv = -0.5*np.einsum('kIba,bIJ->aJk',eris_vcee,r_aaa_ecc_u,optimize = True)
               temp_ecv += 0.5*np.einsum('kabI,bIJ->aJk',eris_veec,r_aaa_ecc_u,optimize = True)
               temp_ecv += 0.5*np.einsum('Ibak,bIJ->aJk',eris_CEev,r_bba_ecc,optimize = True)
               temp_ecv += 0.5*np.einsum('kiba,bJi->aJk',eris_vvee,r_aaa_ecv,optimize = True)
               temp_ecv +=-0.5*np.einsum('kabi,bJi->aJk',eris_veev,r_aaa_ecv,optimize = True)
               temp_ecv += 0.5*np.einsum('ibak,biJ->aJk',eris_VEev,r_bba_evc,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
               
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('KIba,bJI->aJK',eris_ccEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('iKba,bJi->aJK',eris_vcEE,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += 0.5*np.einsum('kIba,bJI->aJk',eris_vcEE,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += 0.5*np.einsum('kiba,bJi->aJk',eris_vvEE,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] += 0.5*np.einsum('KIba,bjI->ajK',eris_ccEE,r_bba_evc,optimize = True).reshape(-1)
               
               temp_ecc = -0.5*np.einsum('KIba,bIJ->aJK',eris_CCEE,r_bbb_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('KabI,bIJ->aJK',eris_CEEC,r_bbb_ecc_u,optimize = True)
               temp_ecc += 0.5*np.einsum('KabI,bIJ->aJK',eris_CEec,r_aab_ecc,optimize = True)
               temp_ecc += 0.5*np.einsum('iKba,bJi->aJK',eris_VCEE,r_bbb_ecv,optimize = True)
               temp_ecc +=-0.5*np.einsum('ibaK,bJi->aJK',eris_VEEC,r_bbb_ecv,optimize = True)
               temp_ecc += 0.5*np.einsum('Kabi,biJ->aJK',eris_CEev,r_aab_evc,optimize = True)
               temp_ecv = -0.5*np.einsum('kIba,bIJ->aJk',eris_VCEE,r_bbb_ecc_u,optimize = True)
               temp_ecv += 0.5*np.einsum('kabI,bIJ->aJk',eris_VEEC,r_bbb_ecc_u,optimize = True)
               temp_ecv += 0.5*np.einsum('kabI,bIJ->aJk',eris_VEec,r_aab_ecc,optimize = True)
               temp_ecv += 0.5*np.einsum('kiba,bJi->aJk',eris_VVEE,r_bbb_ecv,optimize = True)
               temp_ecv +=-0.5*np.einsum('kabi,bJi->aJk',eris_VEEV,r_bbb_ecv,optimize = True)
               temp_ecv += 0.5*np.einsum('kabi,biJ->aJk',eris_VEev,r_aab_evc,optimize = True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)
               
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('KIba,bJI->aJK',eris_CCee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('iKba,bJi->aJK',eris_VCee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('kIba,bJI->aJk',eris_VCee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('kiba,bJi->aJk',eris_VVee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('KIba,bjI->ajK',eris_CCee,r_aab_evc,optimize = True).reshape(-1)

               temp_ecc =   0.5*np.einsum('JIba,bIK->aJK',eris_ccee,r_aaa_ecc_u,optimize = True)
               temp_ecc -=  0.5*np.einsum('JabI,bIK->aJK',eris_ceec,r_aaa_ecc_u,optimize = True)
               temp_ecc -=  0.5*np.einsum('IbaJ,bIK->aJK',eris_CEec,r_bba_ecc,optimize = True)
               temp_ecc += -0.5*np.einsum('iJba,bKi->aJK',eris_vcee,r_aaa_ecv,optimize = True)
               temp_ecc -= -0.5*np.einsum('ibaJ,bKi->aJK',eris_veec,r_aaa_ecv,optimize = True)
               temp_ecc -=  0.5*np.einsum('ibaJ,biK->aJK',eris_VEec,r_bba_evc,optimize = True)
               temp_ecv =   0.5*np.einsum('JIba,bIk->aJk',eris_ccee,r_aaa_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('JabI,bIk->aJk',eris_ceec,r_aaa_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('IbaJ,bIk->aJk',eris_CEec,r_bba_ecv,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
               
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('JIba,bIK->aJK',eris_CCEE,r_bba_ecc,optimize = True).reshape(-1) 
               s[s_bba_ecc:f_bba_ecc] -= 0.5*np.einsum('JabI,bIK->aJK',eris_CEEC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -= 0.5*np.einsum('JabI,bIK->aJK',eris_CEec,r_aaa_ecc_u,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] += 0.5*np.einsum('iJba,biK->aJK',eris_VCEE,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -= 0.5*np.einsum('ibaJ,biK->aJK',eris_VEEC,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_ecc:f_bba_ecc] -=-0.5*np.einsum('Jabi,bKi->aJK',eris_CEev,r_aaa_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += 0.5*np.einsum('JIba,bIk->aJk',eris_CCEE,r_bba_ecv,optimize = True).reshape(-1) 
               s[s_bba_ecv:f_bba_ecv] -= 0.5*np.einsum('JabI,bIk->aJk',eris_CEEC,r_bba_ecv,optimize = True).reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -= 0.5*np.einsum('JabI,bIk->aJk',eris_CEec,r_aaa_ecv,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] += 0.5*np.einsum('jIba,bIK->ajK',eris_VCEE,r_bba_ecc,optimize = True).reshape(-1) 
               s[s_bba_evc:f_bba_evc] -= 0.5*np.einsum('jabI,bIK->ajK',eris_VEEC,r_bba_ecc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -= 0.5*np.einsum('jabI,bIK->ajK',eris_VEec,r_aaa_ecc_u,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] += 0.5*np.einsum('jiba,biK->ajK',eris_VVEE,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -= 0.5*np.einsum('jabi,biK->ajK',eris_VEEV,r_bba_evc,optimize = True).reshape(-1)
               s[s_bba_evc:f_bba_evc] -=-0.5*np.einsum('jabi,bKi->ajK',eris_VEev,r_aaa_ecv,optimize = True).reshape(-1)
               
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('JIba,bIK->aJK',eris_ccee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= 0.5*np.einsum('JabI,bIK->aJK',eris_ceec,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= 0.5*np.einsum('IbaJ,bIK->aJK',eris_CEec,r_bbb_ecc_u,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += 0.5*np.einsum('iJba,biK->aJK',eris_vcee,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -= 0.5*np.einsum('ibaJ,biK->aJK',eris_veec,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_ecc:f_aab_ecc] -=-0.5*np.einsum('ibaJ,bKi->aJK',eris_VEec,r_bbb_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += 0.5*np.einsum('JIba,bIk->aJk',eris_ccee,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= 0.5*np.einsum('JabI,bIk->aJk',eris_ceec,r_aab_ecv,optimize = True).reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= 0.5*np.einsum('IbaJ,bIk->aJk',eris_CEec,r_bbb_ecv,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('jIba,bIK->ajK',eris_vcee,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= 0.5*np.einsum('jabI,bIK->ajK',eris_veec,r_aab_ecc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= 0.5*np.einsum('Ibaj,bIK->ajK',eris_CEev,r_bbb_ecc_u,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] += 0.5*np.einsum('jiba,biK->ajK',eris_vvee,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -= 0.5*np.einsum('jabi,biK->ajK',eris_veev,r_aab_evc,optimize = True).reshape(-1)
               s[s_aab_evc:f_aab_evc] -=-0.5*np.einsum('ibaj,bKi->ajK',eris_VEev,r_bbb_ecv,optimize = True).reshape(-1)

               temp_ecc =   0.5*np.einsum('JIba,bIK->aJK',eris_CCEE,r_bbb_ecc_u,optimize = True)
               temp_ecc -=  0.5*np.einsum('JabI,bIK->aJK',eris_CEEC,r_bbb_ecc_u,optimize = True)
               temp_ecc -=  0.5*np.einsum('JabI,bIK->aJK',eris_CEec,r_aab_ecc,optimize = True)
               temp_ecc += -0.5*np.einsum('iJba,bKi->aJK',eris_VCEE,r_bbb_ecv,optimize = True)
               temp_ecc -= -0.5*np.einsum('ibaJ,bKi->aJK',eris_VEEC,r_bbb_ecv,optimize = True)
               temp_ecc -=  0.5*np.einsum('Jabi,biK->aJK',eris_CEev,r_aab_evc,optimize = True)
               temp_ecv =   0.5*np.einsum('JIba,bIk->aJk',eris_CCEE,r_bbb_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('JabI,bIk->aJk',eris_CEEC,r_bbb_ecv,optimize = True)
               temp_ecv -=  0.5*np.einsum('JabI,bIk->aJk',eris_CEec,r_aab_ecv,optimize = True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)
...
################ ADC(3) i - kja and ajk - i block ############################
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_ecv,eris_ceov,optimize=True)
               ...
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_ecv,eris_ceov,optimize=True)
               ...
               temp_1_ecc = lib.einsum('i,lbik->kbl',r_a, eris_oecc)
               temp_1_ecc -= lib.einsum('i,iblk->kbl',r_a, eris_ceoc)
               ...
               s[s_bbb_ecv:f_bbb_ecv] += 0.5*lib.einsum('bca,pqbc->apq',temp_doubles,t2_1_b[:ncvs,ncvs:,:,:],optimize=True).reshape(-1)
               del temp_singles
               del temp_doubles
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_ecv,eris_CEOV,optimize=True)
               ...
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_ecv,eris_CEOV,optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_evc,eris_OECC,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_evc,eris_CEOC,optimize=True)
               ...
               temp_1_c = lib.einsum('i,lbik->kbl',r_b, eris_OECC)
               temp_1_c -= lib.einsum('i,iblk->kbl',r_b, eris_CEOC)
               ...
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecv,eris_OEcv,optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_evc,eris_OEcc,optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecc,eris_ceOC,optimize=True)
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecc,eris_CEoc,optimize=True)
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_OEcv,optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_OEcc,optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecc,eris_ceOC,optimize=True)
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_oeCV,optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_oeCC,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecc,eris_CEoc,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecv,eris_CEoc,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_evc,eris_CEov,optimize=True)
               ...
               temp_1_c  = lib.einsum('i,lbik->kbl',r_a,eris_oecc)
               temp_1_c  -= lib.einsum('i,iblk->kbl',r_a,eris_ceoc)
...
def ip_cvs_adc_matvec(adc, M_ij=None, eris=None):
    ...
    def sigma_(r):
        ...
        #print("r_aaa_ecv: ", r_aaa_ecv.flags.c_contiguous)          
        #print("r_bba_ecc: ", r_bba_ecc.flags.c_contiguous)
        #print("r_bba_ecv: ", r_bba_ecv.flags.c_contiguous)
        #print("r_bba_evc: ", r_bba_evc.flags.c_contiguous)
        #print("r_aab_ecc: ", r_aab_ecc.flags.c_contiguous)
        #print("r_aab_ecv: ", r_aab_ecv.flags.c_contiguous)
        #print("r_aab_evc: ", r_aab_evc.flags.c_contiguous)
        #print("r_bbb_ecv: ", r_bbb_ecv.flags.c_contiguous)
        #print("r_aaa_ecc_u: ", r_aaa_ecc_u.flags.c_contiguous)        
        #print("r_bbb_ecc_u: ", r_bbb_ecc_u.flags.c_contiguous)
        #exit()  
        ...
        #print("eris_cecc: ", eris_cecc.flags.c_contiguous)        
        #print("eris_cecv: ", eris_cecv.flags.c_contiguous)
        #print("eris_vecc: ", eris_vecc.flags.c_contiguous)
        #print("eris_CECC: ", eris_CECC.flags.c_contiguous)
        #print("eris_CECV: ", eris_CECV.flags.c_contiguous)
        #print("eris_VECC: ", eris_VECC.flags.c_contiguous)
        #print("eris_ceCC: ", eris_ceCC.flags.c_contiguous)
...
############### ADC(3) ajk - bil block ############################
        ...
        if (method == "adc(2)-x" or method == "adc(3)"):
               ...
               #print("eris_cccc: ", eris_cccc.flags.c_contiguous)               
               #print("eris_cccv: ", eris_cccv.flags.c_contiguous)
               #print("eris_cvcv: ", eris_cvcv.flags.c_contiguous)
               #print("eris_ccvv: ", eris_ccvv.flags.c_contiguous)
               #print("eris_CCCC: ", eris_CCCC.flags.c_contiguous)
               #print("eris_CCCV: ", eris_CCCV.flags.c_contiguous)
               #print("eris_CVCV: ", eris_CVCV.flags.c_contiguous)
               #print("eris_CCVV: ", eris_CCVV.flags.c_contiguous)
               #print("eris_ccCC: ", eris_ccCC.flags.c_contiguous)
               #print("eris_cvCC: ", eris_cvCC.flags.c_contiguous)
               #print("eris_ccCV: ", eris_ccCV.flags.c_contiguous)
               #print("eris_vvCC: ", eris_vvCC.flags.c_contiguous)
               #print("eris_cvCV: ", eris_cvCV.flags.c_contiguous)
               #print("eris_ccVV: ", eris_ccVV.flags.c_contiguous)
               #print("eris_cece: ", eris_cece.flags.c_contiguous)
               #print("eris_vece: ", eris_vece.flags.c_contiguous)
               #print("eris_veve: ", eris_veve.flags.c_contiguous)
               #print("eris_CECE: ", eris_CECE.flags.c_contiguous)
               #print("eris_VECE: ", eris_VECE.flags.c_contiguous)
               #print("eris_VEVE: ", eris_VEVE.flags.c_contiguous)
               #print("eris_CEce: ", eris_CEce.flags.c_contiguous)
               #print("eris_VEce: ", eris_VEce.flags.c_contiguous)
               #print("eris_CEve: ", eris_CEve.flags.c_contiguous)
               #print("eris_VEve: ", eris_VEve.flags.c_contiguous)
               #print("eris_ccee: ", eris_ccee.flags.c_contiguous)
               #print("eris_cvee: ", eris_cvee.flags.c_contiguous)
               #print("eris_vvee: ", eris_vvee.flags.c_contiguous)
               #print("eris_CCEE: ", eris_CCEE.flags.c_contiguous)
               #print("eris_CVEE: ", eris_CVEE.flags.c_contiguous)
               #print("eris_VVEE: ", eris_VVEE.flags.c_contiguous)
               #print("eris_ccEE: ", eris_ccEE.flags.c_contiguous)
               ...
               # Einsum paths testing 
               #path_VVEE = np.einsum_path('klba,bJl->aJk',eris_VVEE,r_bbb_ecv,optimize = True)
               #path_VEEV = np.einsum_path('kabl,bJl->aJk',eris_VEVE,r_bbb_ecv,optimize = True)
               #path_VEev = np.einsum_path('kabl,blJ->aJk',eris_VEve,r_aab_evc,optimize = True)
               #print("path info for: ", adc.method_type)
               #print("path info for 'bJl->aJk',eris_VVEE,r_bbb_ecv")
               #print(path_VVEE[0])
               #print(path_VVEE[1])
               #print("path info for 'kabl,bJl->aJk',eris_VEVE,r_bbb_ecv")
               #print(path_VEEV[0])
               #print(path_VEEV[1])
               #print("path info for 'kabl,blJ->aJk',eris_VEve,r_aab_evc")
               #print(path_VEev[0])
               #print(path_VEev[1])
               ...
               #eris_veEV = np.ascontiguousarray(eris_VEve.transpose(3,2,1,0))
               #temp_ecv += 0.5*lib.einsum('kabl,blJ->aJk',eris_veEV,r_bba_evc,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
               ...
               #s[s_aab_evc:f_aab_evc] += 0.5*lib.einsum('jabl,bKl->ajK',eris_veEV,r_bbb_ecv,optimize = True).reshape(-1)
               ...
               #temp_ecv += 0.5*lib.einsum('kabi,biJ->aJk',eris_veEV,r_bba_evc,optimize = True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1]].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)
               ...
               #s[s_aab_evc:f_aab_evc] -=-0.5*lib.einsum('jabi,bKi->ajK',eris_veEV,r_bbb_ecv,optimize = True).reshape(-1)
        ...
        if (method == "adc(3)"):
               ...
               #print("eris_oecc: ", eris_oecc.flags.c_contiguous)
               #print("eris_ceoc: ", eris_ceoc.flags.c_contiguous)
               #print("eris_oecv: ", eris_oecv.flags.c_contiguous)
               #print("eris_ceov: ", eris_ceov.flags.c_contiguous)
               #print("eris_OECC: ", eris_OECC.flags.c_contiguous)
               #print("eris_CEOC: ", eris_CEOC.flags.c_contiguous)
               #print("eris_OECV: ", eris_OECV.flags.c_contiguous)
               #print("eris_CEOV: ", eris_CEOV.flags.c_contiguous)
...
################ ADC(3) i - kja and ajk - i block ############################
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blk,Iblk->I',temp_ecv_1,eris_ceov,optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blK,lbIK->I',temp_ecv_2,eris_oecc,optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blK,IblK->I',temp_ecv_2,eris_ceoc,optimize=True)
               ...
               s[s_a:f_a] +=0.5*lib.einsum('blj,Iblj->I',temp_ecv_1,eris_ceov,optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blJ,lbIJ->I',temp_ecv_2,eris_oecc,optimize=True)
               s[s_a:f_a] +=0.5*lib.einsum('blJ,IblJ->I',temp_ecv_2,eris_ceoc,optimize=True)
               ...
               temp_1 -= lib.einsum('I,IblK->Kbl',r_a, eris_ceoc)
               temp_2 = lib.einsum('I,lbIk->kbl',r_a, eris_oecv)
               temp_2 -= lib.einsum('I,Iblk->kbl',r_a, eris_ceov)
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blk,Iblk->I',temp_ecv_1,eris_CEOV,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blK,lbIK->I',temp_ecv_2,eris_OECC,optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blK,IblK->I',temp_ecv_2,eris_CEOC,optimize=True)
               ...
               s[s_b:f_b] += 0.5*lib.einsum('blj,Iblj->I',temp_ecv_1,eris_CEOV,optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blJ,lbIJ->I',temp_ecv_2,eris_OECC,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blJ,IblJ->I',temp_ecv_2,eris_CEOC,optimize=True)
               ...
               temp_1 -= lib.einsum('I,IblK->Kbl',r_b, eris_CEOC)
               temp_2 = lib.einsum('I,lbIk->kbl',r_b, eris_OECV)
               temp_2 -= lib.einsum('I,Iblk->kbl',r_b, eris_CEOV)
               ...
               temp_2  = lib.einsum('I,lbIK->Kbl',r_a,eris_oecc)
               temp_2  -= lib.einsum('I,IblK->Kbl',r_a,eris_ceoc)
               temp_3  = lib.einsum('I,lbIk->kbl',r_a,eris_oecv)
               temp_3  -= lib.einsum('I,Iblk->kbl',r_a,eris_ceov)
               ...
               temp_1  = -lib.einsum('I,IblJ->Jbl',r_a,eris_ceOC,optimize=True)
               temp_2  = -lib.einsum('I,Iblj->jbl',r_a,eris_ceOV,optimize=True)
               ...
               temp_3  = -lib.einsum('I,IblJ->Jbl',r_b,eris_CEoc,optimize=True)
               temp_4  = -lib.einsum('I,Iblj->jbl',r_b,eris_CEov,optimize=True)
    ...
    return sigma_
def ip_cvs_adc_matvec_off_sliced(adc, M_ij=None, eris=None):
    # Sliced integrals 
    if adc.method not in ("adc(2)", "adc(2)-x", "adc(3)"):
        raise NotImplementedError(adc.method)
...
################ ADC(3) i - kja and ajk - i block ############################
               ...
               #for p in range(0,ncvs,chnk_size):
                   ...
                   #temp_singles[a:a+k] += 0.5*lib.einsum('abc,icab->i',temp_1, eris_ovvv, optimize=True)
                   #temp_singles[a:a+k] -= 0.5*lib.einsum('abc,ibac->i',temp_1, eris_ovvv, optimize=True)
                   temp_singles[a:a+k] += 0.5*lib.einsum('abc,icab->i',temp_1_ecc, eris_ovvv[:ncvs,:,:,:], optimize=True)
                   temp_singles[a:a+k] -= 0.5*lib.einsum('abc,ibac->i',temp_1_ecc, eris_ovvv[:ncvs,:,:,:], optimize=True)
                   temp_singles[a:a+k] += 0.5*lib.einsum('abc,icab->i',temp_1_ecv, eris_ovvv[:ncvs,:,:,:], optimize=True)
                   temp_singles[a:a+k] -= 0.5*lib.einsum('abc,ibac->i',temp_1_ecv, eris_ovvv[:ncvs,:,:,:], optimize=True)

                   #temp_doubles += lib.einsum('i,icab->bca',r_a[a:a+k],eris_ovvv,optimize=True)
                   #temp_doubles -= lib.einsum('i,ibac->bca',r_a[a:a+k],eris_ovvv,optimize=True)
                   temp_doubles += lib.einsum('i,icab->bca',r_a[a:a+k],eris_ovvv[:ncvs,:,:,:],optimize=True)
                   temp_doubles -= lib.einsum('i,ibac->bca',r_a[a:a+k],eris_ovvv[:ncvs,:,:,:],optimize=True)
                   del eris_ovvv
                   a += k
               ...
               #s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp,eris_ovoo,optimize=True)
               #s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp,eris_ovoo,optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_ecc,eris_ovoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_ecc,eris_ovoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_ecv,eris_ovoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_ecv,eris_ovoo[:ncvs,:,:,ncvs:],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_evc,eris_ovoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_evc,eris_ovoo[:ncvs,:,:,:ncvs],optimize=True)
               ...
               #s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1,eris_ovOO,optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecc,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecv,eris_ovOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1_evc,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_ecc,eris_ovoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_ecc,eris_ovoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_ecv,eris_ovoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_ecv,eris_ovoo[:ncvs,:,:,ncvs:],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_evc,eris_ovoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_evc,eris_ovoo[:ncvs,:,:,:ncvs],optimize=True)
               ...
               #s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1,eris_ovOO,optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecc,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_ovOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)

               #temp_1 = lib.einsum('i,lbik->kbl',r_a, eris_ovoo)
               #temp_1 -= lib.einsum('i,iblk->kbl',r_a, eris_ovoo)
               temp_1_ecc = lib.einsum('i,lbik->kbl',r_a, eris_ovoo[:,:,:ncvs,:ncvs])
               temp_1_ecc -= lib.einsum('i,iblk->kbl',r_a, eris_ovoo[:ncvs,:,:,:ncvs])
               temp_1_ecv = lib.einsum('i,lbik->kbl',r_a, eris_ovoo[:,:,:ncvs,ncvs:])
               temp_1_ecv -= lib.einsum('i,iblk->kbl',r_a, eris_ovoo[:ncvs,:,:,ncvs:])
               ...
               #temp_2  = lib.einsum('i,lbik->kbl',r_b,eris_ovOO)
               temp_2  = lib.einsum('i,lbik->kbl',r_b,eris_ovOO[:,:,:ncvs,:])
               #temp = lib.einsum('kbl,jlab->ajk',temp_2,t2_1_a,optimize=True)
               temp_ecc = lib.einsum('kbl,jlab->ajk',temp_2[:ncvs,:,:],t2_1_a[:ncvs,:,:,:],optimize=True)
               temp_ecv = lib.einsum('kbl,jlab->ajk',temp_2[ncvs:,:,:],t2_1_a[:ncvs,:,:,:],optimize=True)
               temp_evc = lib.einsum('kbl,jlab->ajk',temp_2[:ncvs],t2_1_a[ncvs:,:,:,:],optimize=True)
               #s[s_aba:f_aba] += temp.reshape(-1)
               s[s_aab_ecc:f_aab_ecc] += temp_ecc.reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += temp_ecv.reshape(-1)
               s[s_aab_evc:f_aab_evc] += temp_evc.reshape(-1)
               ...
               #temp_1 = lib.einsum('i,lbij->jbl',r_a, eris_ovoo)
               #temp_1 -= lib.einsum('i,iblj->jbl',r_a, eris_ovoo)
               temp_1 = lib.einsum('i,lbij->jbl',r_a, eris_ovoo[:,:,:ncvs,:])
               temp_1 -= lib.einsum('i,iblj->jbl',r_a, eris_ovoo[:ncvs,:,:,:])

               #temp  = lib.einsum('jbl,klab->ajk',temp_1,t2_1_a,optimize=True)
               temp_ecc  = lib.einsum('jbl,klab->ajk',temp_1[:ncvs,:,:],t2_1_a[:ncvs,:,:,:],optimize=True)
               temp_ecv  = lib.einsum('jbl,klab->ajk',temp_1[:ncvs,:,:],t2_1_a[ncvs:,:,:,:],optimize=True)
               #s[s_aaa:f_aaa] -= temp[:,ij_ind_a[0],ij_ind_a[1] ].reshape(-1)
               s[s_aaa_ecc:f_aaa_ecc] -= temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] -= temp_ecv.reshape(-1)
               ...
               for p in range(0,nocc_b,chnk_size):
                   ...
                   temp_doubles += lib.einsum('i,icab->bca',r_b[a:a+k],eris_OVVV[:ncvs,:,:,:],optimize=True)
                   temp_doubles -= lib.einsum('i,ibac->bca',r_b[a:a+k],eris_OVVV[:ncvs,:,:,:],optimize=True)
                   del eris_OVVV
                   a += k
               ...
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecv,eris_OVoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_evc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               ...
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_ecc,eris_OVOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_ecc,eris_OVOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_ecv,eris_OVOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_ecv,eris_OVOO[:ncvs,:,:,ncvs:],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_evc,eris_OVOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_evc,eris_OVOO[:ncvs,:,:,:ncvs],optimize=True)
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_OVoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_ecc,eris_OVOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_ecc,eris_OVOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_ecv,eris_OVOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_ecv,eris_OVOO[:ncvs,:,:,ncvs:],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_evc,eris_OVOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_evc,eris_OVOO[:ncvs,:,:,:ncvs],optimize=True)

               temp_2  = lib.einsum('i,lbik->kbl',r_a,eris_OVoo[:,:,:ncvs,:])
               temp_ecc = lib.einsum('kbl,jlab->ajk',temp_2[:ncvs,:,:],t2_1_b[:ncvs,:,:,:],optimize=True)
               temp_ecv = lib.einsum('kbl,jlab->ajk',temp_2[ncvs:,:,:],t2_1_b[:ncvs,:,:,:],optimize=True)
               temp_evc = lib.einsum('kbl,jlab->ajk',temp_2[:ncvs,:,:],t2_1_b[ncvs:,:,:,:],optimize=True)
               s[s_bba_ecc:f_bba_ecc] += temp_ecc.reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += temp_ecv.reshape(-1)
               s[s_bba_evc:f_bba_evc] += temp_evc.reshape(-1)

               temp_1 = lib.einsum('i,lbik->kbl',r_b, eris_OVOO[:,:,:ncvs,:])
               temp_1 -= lib.einsum('i,iblk->kbl',r_b, eris_OVOO[:ncvs,:,:,:])

               temp_ecc  = lib.einsum('kbl,jlab->ajk',temp_1[:ncvs,:,:],t2_1_b[:ncvs,:,:,:],optimize=True)
               temp_ecv  = lib.einsum('kbl,jlab->ajk',temp_1[ncvs:,:,:],t2_1_b[:ncvs,:,:,:],optimize=True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)

               temp_1 = lib.einsum('i,lbij->jbl',r_b, eris_OVOO[:,:,:ncvs,:])
               temp_1 -= lib.einsum('i,iblj->jbl',r_b, eris_OVOO[:ncvs,:,:,:])

               temp_ecc  = lib.einsum('jbl,klab->ajk',temp_1[:ncvs,:,:],t2_1_b[:ncvs,:,:,:],optimize=True)
               temp_ecv  = lib.einsum('jbl,klab->ajk',temp_1[:ncvs,:,:],t2_1_b[ncvs:,:,:,:],optimize=True)
               s[s_bbb_ecc:f_bbb_ecc] -= temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] -= temp_ecv.reshape(-1)
               del t2_1_b
               ...
               #for p in range(0,ncvs,chnk_size):
                   ...
                   s[s_a:f_a][a:a+k] += lib.einsum('abc,icab->i',temp_1_ecc, eris_ovVV[:ncvs,:,:,:], optimize=True)
                   s[s_a:f_a][a:a+k] += lib.einsum('abc,icab->i',temp_1_ecv, eris_ovVV[:ncvs,:,:,:], optimize=True)
                   s[s_a:f_a][a:a+k] += lib.einsum('abc,icab->i',temp_1_evc, eris_ovVV[:ncvs,:,:,:], optimize=True)

                   temp_2 += lib.einsum('i,icab->cba',r_a[a:a+k],eris_ovVV[:ncvs,:,:,:],optimize=True)
                   del eris_ovVV
                   a += k
               ...
               for p in range(0,ncvs,chnk_size):
                   if getattr(adc, 'with_df', None):
                       ...
                       eris_OVvv = radc_ao2mo.unpack_eri_1(eris.OVvv, nvir_a)
                   k = eris_OVvv.shape[0]
                   s[s_b:f_b][a:a+k] += lib.einsum('abc,icab->i',temp_1_ecc, eris_OVvv[:ncvs,:,:,:], optimize=True)
                   s[s_b:f_b][a:a+k] += lib.einsum('abc,icab->i',temp_1_ecv, eris_OVvv[:ncvs,:,:,:], optimize=True)
                   s[s_b:f_b][a:a+k] += lib.einsum('abc,icab->i',temp_1_evc, eris_OVvv[:ncvs,:,:,:], optimize=True)

                   temp_2 += lib.einsum('i,icab->bca',r_b[a:a+k],eris_OVvv[:ncvs,:,:,:],optimize=True)
                   del eris_OVvv
                   a += k
               ...
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecv,eris_OVoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blk,lbik->i',temp_1_evc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecc,eris_ovOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecv,eris_ovOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blk,iblk->i',temp_2_evc,eris_ovOO[:ncvs,:,:,ncvs:],optimize=True)
               ...
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1_ecv,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blk,lbik->i',temp_1_evc,eris_ovOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecc,eris_OVoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_2_ecv,eris_OVoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blk,iblk->i',temp_2_evc,eris_OVoo[:ncvs,:,:,ncvs:],optimize=True)
               ...
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_OVoo[:,:,:ncvs,ncvs:],optimize=True)
               s[s_a:f_a] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_OVoo[:,:,:ncvs,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecc,eris_ovOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecv,eris_ovOO[:ncvs,:,:,:ncvs],optimize=True)
               s[s_a:f_a] += 0.5*lib.einsum('blj,iblj->i',temp_2_evc,eris_ovOO[:ncvs,:,:,ncvs:],optimize=True)
               ...
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_ecv,eris_ovOO[:,:,:ncvs,ncvs:],optimize=True)
               s[s_b:f_b] -= 0.5*lib.einsum('blj,lbij->i',temp_1_evc,eris_ovOO[:,:,:ncvs,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecc,eris_OVoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_ecv,eris_OVoo[:ncvs,:,:,:ncvs],optimize=True)
               s[s_b:f_b] += 0.5*lib.einsum('blj,iblj->i',temp_2_evc,eris_OVoo[:ncvs,:,:,ncvs:],optimize=True)
               
               temp_2 = lib.einsum('i,lbik->kbl',r_a, eris_OVoo[:,:,:ncvs,:])
               temp_ecc = lib.einsum('kbl,jlab->ajk',temp_2[:ncvs,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               temp_ecv = lib.einsum('kbl,jlab->ajk',temp_2[ncvs:,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               s[s_aaa_ecc:f_aaa_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] += temp_ecv.reshape(-1)

               temp_1  = lib.einsum('i,lbik->kbl',r_a,eris_ovoo[:,:,:ncvs,:])
               temp_1  -= lib.einsum('i,iblk->kbl',r_a,eris_ovoo[:ncvs,:,:,:])

               temp_ecc  = lib.einsum('kbl,ljba->ajk',temp_1[:ncvs,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               temp_ecv  = lib.einsum('kbl,ljba->ajk',temp_1[ncvs:,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               temp_evc  = lib.einsum('kbl,ljba->ajk',temp_1[:ncvs,:,:],t2_1_ab[:,ncvs:,:,:],optimize=True)
               s[s_bba_ecc:f_bba_ecc] += temp_ecc.reshape(-1)
               s[s_bba_ecv:f_bba_ecv] += temp_ecv.reshape(-1)
               s[s_bba_evc:f_bba_evc] += temp_evc.reshape(-1)
               ...
               temp_ecc = lib.einsum('kbl,ljba->ajk',temp_2[:ncvs,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               temp_ecv = lib.einsum('kbl,ljba->ajk',temp_2[ncvs:,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               s[s_bbb_ecc:f_bbb_ecc] += temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] += temp_ecv.reshape(-1)
               ...
               temp_ecc  = lib.einsum('kbl,jlab->ajk',temp_1[:ncvs,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               temp_ecv  = lib.einsum('kbl,jlab->ajk',temp_1[ncvs:,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               temp_evc  = lib.einsum('kbl,jlab->ajk',temp_1[:ncvs,:,:],t2_1_ab[ncvs:,:,:,:],optimize=True)
               s[s_aab_ecc:f_aab_ecc] += temp_ecc.reshape(-1)
               s[s_aab_ecv:f_aab_ecv] += temp_ecv.reshape(-1)
               s[s_aab_evc:f_aab_evc] += temp_evc.reshape(-1)

               temp_2 = lib.einsum('i,lbij->jbl',r_a, eris_OVoo[:,:,:ncvs,:])

               temp_ecc = lib.einsum('jbl,klab->ajk',temp_2[:ncvs,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               temp_ecv = lib.einsum('jbl,klab->ajk',temp_2[:ncvs,:,:],t2_1_ab[ncvs:,:,:,:],optimize=True)
               s[s_aaa_ecc:f_aaa_ecc] -= temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_aaa_ecv:f_aaa_ecv] -= temp_ecv.reshape(-1)

               temp  = -lib.einsum('i,iblj->jbl',r_a,eris_ovOO[:ncvs,:,:,:],optimize=True)
               temp_1_ecc = -lib.einsum('jbl,klba->ajk',temp[:ncvs,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               temp_1_ecv = -lib.einsum('jbl,klba->ajk',temp[:ncvs,:,:],t2_1_ab[ncvs:,:,:,:],optimize=True)
               temp_1_evc = -lib.einsum('jbl,klba->ajk',temp[ncvs:,:,:],t2_1_ab[:ncvs,:,:,:],optimize=True)
               s[s_bba_ecc:f_bba_ecc] -= temp_1_ecc.reshape(-1)
               s[s_bba_ecv:f_bba_ecv] -= temp_1_ecv.reshape(-1)
               s[s_bba_evc:f_bba_evc] -= temp_1_evc.reshape(-1)

               temp_2 = lib.einsum('i,lbij->jbl',r_b, eris_ovOO[:,:,:ncvs,:])
               temp_ecc = lib.einsum('jbl,lkba->ajk',temp_2[:ncvs,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               temp_ecv = lib.einsum('jbl,lkba->ajk',temp_2[:ncvs,:,:],t2_1_ab[:,ncvs:,:,:],optimize=True)
               s[s_bbb_ecc:f_bbb_ecc] -= temp_ecc[:,ij_ind_ncvs[0],ij_ind_ncvs[1] ].reshape(-1)
               s[s_bbb_ecv:f_bbb_ecv] -= temp_ecv.reshape(-1)

               temp  = -lib.einsum('i,iblj->jbl',r_b,eris_OVoo[:ncvs,:,:,:],optimize=True)
               temp_1_ecc = -lib.einsum('jbl,lkab->ajk',temp[:ncvs,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               temp_1_ecv = -lib.einsum('jbl,lkab->ajk',temp[:ncvs,:,:],t2_1_ab[:,ncvs:,:,:],optimize=True)
               temp_1_evc = -lib.einsum('jbl,lkab->ajk',temp[ncvs:,:,:],t2_1_ab[:,:ncvs,:,:],optimize=True)
               s[s_aab_ecc:f_aab_ecc] -= temp_1_ecc.reshape(-1)
               s[s_aab_ecv:f_aab_ecv] -= temp_1_ecv.reshape(-1)
               s[s_aab_evc:f_aab_evc] -= temp_1_evc.reshape(-1)
               del t2_1_ab
