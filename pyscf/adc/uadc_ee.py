...
#TPDM ADC(2)-X
        ...
        if adc.spin_c is True or adc.tpdm is True:
            ...
            spin_c = 0.25*((nocc_a - nocc_b)**2) + 0.5*(nocc_a + nocc_b) + np.einsum('pr,qs,pqrs->',delta[:nocc_a,:nocc_b], delta[:nocc_a,:nocc_b].T, IjlK, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[nocc_a:,nocc_b:], delta[nocc_a:,nocc_b:].T, AbdC, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[:nocc_a,nocc_b:], delta[nocc_a:,:nocc_b].T, IjdC, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[nocc_a:,:nocc_b], delta[:nocc_a,nocc_b:].T,AblK, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[:nocc_a,nocc_b:], delta[:nocc_a,nocc_b:].T,IadK, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[:nocc_a,:nocc_b], delta[nocc_a:,nocc_b:].T,IakD, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[nocc_a:,nocc_b:], delta[:nocc_a,:nocc_b:].T,AidK, optimize = True)
            spin_c += np.einsum('pr,qs,pqrs->', delta[nocc_a:,:nocc_b], delta[nocc_a:,:nocc_b].T,AikD, optimize = True)
    ...
    if adc.opdm is True:
        ...
        opdm_a = np.array([])
        np.append(opdm_a,temp_a)

        opdm_b = np.array([])
        np.append(opdm_b,temp_b)

        pdm = (opdm_a,opdm_b, None ,None ,None)

    if adc.tpdm is True:
        tpdm_a = np.array([])
        np.append(tpdm_a,temp_2a)

        tpdm_b = np.array([])
        np.append(tpdm_b,temp_2b)
        pdm = (None ,None ,tpdm_a,tpdm_b,abab)

    if adc.opdm is True and adc.tpdm is True:

        spin = spin_c 
        spin = np.array([])
        np.append(spin,spin_c)
        ...
        pdm = (temp_a ,temp_b ,temp_2a,temp_2b ,abab)
