...
####
####    if chnk_size == 1:
####        Lvv_temp = np.ascontiguousarray(Lvv.T[p].reshape(-1,naux))
####        Lvv = Lvv.reshape(naux,nvir*nvir)
####        vvvv = np.dot(Lvv_temp, Lvv)
####        exch = np.dot(Lvv.T, Lvv_temp.T)
####        exch = exch.reshape(nvir, nvir, nvir)
#####        vvvv = -vvvv.T.reshape(nvir, nvir, nvir)
#####        vvvv -= vvvv.transpose(0,2,1)
#####        vvvv = vvvv.reshape(1, nvir, nvir, nvir)
####        vvvv = vvvv.reshape(nvir, nvir, nvir).transpose(1,0,2)
####        vvvv -= exch
####        vvvv = vvvv.reshape(1, nvir, nvir, nvir)
####
####    else:
